...
import { getSingleUser,updateUser } from "../redux/action_creators";
...
function Edituser(){
    ...
    const {id} = useParams();
    const {user} = useSelector((state)=>{return state.usersData});
    useEffect(()=>{
...
const address=stateuser.address;*/
   ...
   const handleSubmit = (event)=>{
        ...
        if(!name || !email || !contact || !address ){
            ...
            dispatch(updateUser(stateuser,id));
            navigate("/");
            seterror("");
    ...
    return(
        ...
        <ButtonGroup variant="contained" aria-label="outlined button group">
        <Button type='submit' color='primary' style={{marginTop:"5px"}}>Update User </Button>
        </ButtonGroup>
        </form>
        </div>
        </div>
